                ...
                <tr className={`${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-blue-50 border-b border-gray-100`} data-testid={`node-row-${node.id}`}>
                  ...
                  <td className="px-2 py-2 whitespace-nowrap text-sm text-gray-900">
                    {node.scamalytics_risk ? (
                      <Badge 
                        variant={
                          node.scamalytics_risk === 'low' ? 'success' : 
                          node.scamalytics_risk === 'medium' ? 'warning' : 
                          node.scamalytics_risk === 'high' ? 'destructive' : 
                          node.scamalytics_risk === 'critical' ? 'destructive' : 
                          'default'
                        }
                        className={
                          node.scamalytics_risk === 'low' ? 'bg-green-100 text-green-800 border-green-300' : 
                          node.scamalytics_risk === 'medium' ? 'bg-yellow-100 text-yellow-800 border-yellow-300' : 
                          node.scamalytics_risk === 'high' ? 'bg-orange-100 text-orange-800 border-orange-300' : 
                          node.scamalytics_risk === 'critical' ? 'bg-red-100 text-red-800 border-red-300' : 
                          ''
                        }
                      >
